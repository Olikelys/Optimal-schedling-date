...
function[bestX,fMin,avgen,CSOs]= ICSO(M,pop,dim,ub,lb,rPercent,hPercent,mPercent,FitFunc,G)%(genn,PS,dim,upbnd,lwbnd,rP,hP,mP,FitFunc,G);
...
pFit = fit; % The individual's best fitness value     个体最优值
...
for t = 1 : M    
   ...
   for i = 1 : rNum                % Update the rNum roosters' values.      更新rnum公鸡的价值
        ...
        anotherRooster = randiTabu(1,rNum,i);  
        if(pFit(sortIndex(i)) <= pFit(sortIndex(anotherRooster)))
            tempSigma = 1;
        else
            tempSigma = exp((pFit(sortIndex(anotherRooster)) - pFit(sortIndex(i)))/(abs(pFit(sortIndex(i)))+ realmin));
        end
   ...
   for i = (rNum + 1 ):(rNum + hNum)  % Update the hNum hens' values.
        other = randiTabu(1,i,mate(i-rNum));  
        ...
        % (th) chicken.+(alfa*(pX(sortIndex(mother(i-rNum-hNum)),:)-pX(sortIndex(i),:))+(1-alfa)*(Q((i-rNum-hNum),:)-pX(sortIndex(i),:))).* FL(i);
        
        c1 = exp((pFit(sortIndex(i))-pFit(sortIndex(mate(i-rNum))))/(abs(pFit(sortIndex(i)))+realmin));          
        c2 = exp((-pFit(sortIndex(i)) + pFit(sortIndex(other))));

        % x(sortIndex(i),:)=pX(sortIndex(i),:)+(alfa*(pX(sortIndex(mate(i-rNum)),:)- pX(sortIndex(i),:))+(1-alfa)*(Q(i,:)-pX(sortIndex(i),:))).* c1.*rand(1,dim) + (pX(sortIndex(other),:) -pX(sortIndex(i),:)).*c2.*rand(1,dim); 
        x(sortIndex(i),:)=pX(sortIndex(i),:)+(pX(sortIndex(mate(i-rNum)),:)- pX(sortIndex(i),:)).* c1.*rand(1,dim) + (pX(sortIndex(other),:) -pX(sortIndex(i),:)).*c2.*rand(1,dim); 
        x(sortIndex(i),:) = Bounds(x(sortIndex(i),:), lb, ub);
        fit(sortIndex(i)) = FitFunc(x(sortIndex(i),:));
    end
    ...
    for i = (rNum + hNum +1) : pop    % Update the cNum chicks' values.
        ...
        x(sortIndex(i),:) = pX(sortIndex(i),:)+(alfa*(pX(sortIndex(mother(i-rNum-hNum)),:)-pX(sortIndex(i),:))+(1-alfa)*(Q(i,:)-pX(sortIndex(i),:))).* FL(i);
        x(sortIndex(i),:) = Bounds(x(sortIndex(i),:),lb,ub);
        fit(sortIndex(i)) = FitFunc(x(sortIndex(i),:)); 
    end
...
%  the value of "tabu"
function value = randiTabu(min,max,tabu)  %other = (1,rNum,i,1);  
value = ones(1,1).* max.*2;
num = 1;
while (num<= 1)
    temp = randi([min, max],1,1);    
    if(length(find(value ~= temp)) == 1 && temp~=tabu)
        value(num) = temp;
        num = num + 1;
    end
end
